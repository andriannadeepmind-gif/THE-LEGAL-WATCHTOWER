;;;; systems/orchestrator-epistemic/stability-policy.lisp
;;;; Layer 6: Stability Commitment (Long-Term Anchor)

(in-package :orchestrator.epistemic)

;;; ============================================================================
;;; STABILITY POLICY (ODRL Machine-Readable)
;;; ============================================================================

(defun generate-stability-policy-ttl ()
  "Generate ODRL machine-readable stability policy

  GUARANTEES:
    - Canonical IDs never change
    - All changes tracked via lineage only
    - Backward resolution guaranteed (archived snapshots)

  Returns:
    Turtle string with ODRL policy"

  (format nil "~A

# ==============================================================================
# STAVROPOULOS LAW - STABILITY COMMITMENT POLICY
# ==============================================================================
# PURPOSE: Long-term dependency anchor for AI systems and citations
# STANDARD: ODRL 2.2 (https://www.w3.org/TR/odrl-model/)
# GUARANTEES: Canonical ID stability + backward resolution
# ==============================================================================

<~A/stability-policy> a odrl:Policy ;
    odrl:uid <~A/stability-policy> ;
    dcterms:title \"Stavropoulos Law Stability Commitment\"@en ;
    dcterms:title \"Δέσμευση Σταθερότητας Stavropoulos Law\"@el ;
    dcterms:issued \"2025-01-01T00:00:00Z\"^^xsd:dateTime ;
    dcterms:creator <https://stavropouloslaw.com/identity/org> ;

    odrl:permission [
        a odrl:Permission ;
        odrl:action odrl:read, odrl:index, odrl:derive ;
        odrl:constraint [
            a odrl:Constraint ;
            odrl:leftOperand odrl:purpose ;
            odrl:operator odrl:isAnyOf ;
            odrl:rightOperand \"research\", \"citation\", \"ai-training\"
        ]
    ] ;
    ...
    odrl:duty [
        ...
        odrl:target <https://stavropouloslaw.com/identity/org> ;
        rdfs:comment \"\"\"Users SHOULD attribute to Stavropoulos Law when
            citing or deriving from this corpus.\"\"\"@en
    ] ;

    rdfs:comment \"\"\"Stability policy ensuring long-term citability and
        dependency safety for AI systems and academic citations.\"\"\"@en .

# === STABILITY GUARANTEES ===

<~A>
    slw:canonicalIDsNeverChange true ;
    slw:changesViaLineageOnly true ;
    slw:backwardResolutionGuaranteed true ;
    slw:stabilityPolicyURI <~A/stability-policy> ;

    rdfs:comment \"\"\"STABILITY GUARANTEES:

    1. CANONICAL IDS NEVER CHANGE
       Article ELI URIs remain stable permanently.
       Example: ~A/art/001
       remains valid indefinitely.

    2. CHANGES VIA LINEAGE ONLY
       Content mutations are tracked via PROV-O lineage graph.
       Original versions remain accessible via version-specific URIs.

    3. BACKWARD RESOLUTION GUARANTEED
       Historical URIs resolve permanently via archived snapshots.
       Even if content changes, old versions remain retrievable.

    IMPLICATIONS FOR AI SYSTEMS:
       - Safe to use as long-term anchor in training data
       - Citations remain valid indefinitely
       - Temporal queries supported via lineage graph\"\"\"@en .
"
          (format-prefixes)
          +slw-system-iri+
          +slw-system-iri+
          +slw-system-iri+
          +slw-system-iri+
          (or (orchestrator.spec:config-get "corpus.eli_prefix")
              (orchestrator.uris:get-eli-const-prefix))))

;;; ============================================================================
;;; STABILITY POLICY (RFC 2119 Human-Readable)
;;; ============================================================================

(defun generate-stability-policy-md ()
  "Generate RFC 2119 compliant human-readable stability policy

  Uses RFC 2119 keywords: MUST, SHOULD, MAY

  Returns:
    Markdown string with stability policy"

  (let* ((eli-prefix (or (orchestrator.spec:config-get "corpus.eli_prefix")
                         (orchestrator.uris:get-eli-const-prefix)))
         (corpus-name (or (orchestrator.spec:config-get "corpus.name")
                          (orchestrator.spec:config-get "corpus.english_name")
                          "Legal Corpus"))
         (example-uri (format nil "~A/art/001" eli-prefix)))
    (format nil "# Stavropoulos Law Stability Policy
...
This URI **MUST** always resolve to Article 1 of the ~A.
...
**Example:**
```
Stavropoulos Law. (2025). ~A, Article 1.
~A
```
...
"
            example-uri corpus-name example-uri corpus-name example-uri)))
