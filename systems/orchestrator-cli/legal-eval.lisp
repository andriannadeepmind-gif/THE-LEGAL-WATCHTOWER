...
(defparameter +legal-eval-cases+
  '((:id "klopi-full" :tags (:core :pk-372)
    ...
    ;; ── HELD-OUT [0075 verify]: ΙΔΙΕΣ έννοιες, ΔΙΑΦΟΡΕΤΙΚΗ διατύπωση — μετρούν
    ;;    ΓΕΝΙΚΕΥΣΗ. Η αντιπαλική επιθεώρηση έδειξε ότι το exact-substring MWE ΔΕΝ
    ;;    γενικεύει· αυτά το κάνουν ΟΡΑΤΟ αντί να το κρύβουν (τίμια = ο αριθμός
    ;;    μετρά ικανότητα, όχι in-distribution capture).
    (:id "consent-paraphrase" :tags (:held-out :pk-372 :defeater)
     :narrative "Ο δράστης αφαίρεσε τα ξένα κινητά εργαλεία με σκοπό την παράνομη ιδιοποίηση, αλλά είχε τη συγκατάθεση του ιδιοκτήτη."
     :gold-facts ((:γεγονός :Α :αφαιρεί :εργαλεία) (:γεγονός :εργαλεία :είναι :κινητό)
                  (:γεγονός :εργαλεία :είναι :ξένο) (:γεγονός :Α :σκοπός :παράνομη-ιδιοποίηση)
                  (:γεγονός :Α :έχει-συναίνεση-κατόχου :εργαλεία))
     :expect ((:norm-klopi-372 . :out)))    ; η γείωση ΔΕΝ πιάνει το παράφρασμα (τίμιο fail)

    (:id "defeater-inflected" :tags (:held-out :pk-299 :defeater)
     :narrative "Ο δράστης θανάτωσε τον παθόντα με δόλο, ενώ βρισκόταν σε κατάσταση νόμιμης άμυνας."
     :gold-facts ((:γεγονός :Β :θανατώνει :Γ) (:γεγονός :Β :ενεργεί-με :δόλο)
                  (:γεγονός :Β :τελεί-σε :νόμιμη-άμυνα))
     :expect ((:norm-anthropoktonia-299 . :out)))  ; κλίση «νόμιμης άμυνας» ≠ span (τίμιο fail)

    (:id "consent-other-object" :tags (:held-out :pk-372 :no-fabrication)
     :narrative "Ο δράστης αφαίρεσε τα ξένα κινητά εργαλεία με σκοπό την παράνομη ιδιοποίηση, αλλά είχε τη συναίνεση του κατόχου για το αυτοκίνητο."
     :gold-facts ((:γεγονός :Α :αφαιρεί :εργαλεία) (:γεγονός :εργαλεία :είναι :κινητό)
                  (:γεγονός :εργαλεία :είναι :ξένο) (:γεγονός :Α :σκοπός :παράνομη-ιδιοποίηση))
     :expect ((:norm-klopi-372 . :in))))    ; η κλοπή ΤΩΝ ΕΡΓΑΛΕΙΩΝ στέκει — καμία εφεύρεση συναίνεσης
  "Το gold benchmark (σπόρος, αυξάνεται). Κάθε υπόθεση: αφήγηση + ΑΝΕΞΑΡΤΗΤΑ
   ...
   δύσκολη γείωση· :held-out = ΓΕΝΙΚΕΥΣΗ (παράφραση/κλίση/άλλο-αντικείμενο) που
   ΕΚΘΕΤΕΙ το ταβάνι του exact-substring — ο αριθμός μετρά ικανότητα, όχι capture.")
