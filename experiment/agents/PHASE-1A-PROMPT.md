...
## ΣΤΑΔΙΑΚΟ CHECKPOINT (προσθήκη v2 — εντολή δημιουργού)
Μια προηγούμενη εκτέλεση χάθηκε ολόκληρη σε session limit με ΜΗΔΕΝ παραδοτέα.
Δεν επαναλαμβάνεται:

- **Γράψε το αρχείο σου ΝΩΡΙΣ και ΞΑΝΑΓΡΑΨΕ το ΣΥΧΝΑ** — μετά τα πρώτα ~10
  αρχεία που διαβάζεις, και έπειτα κάθε ~15-20 αρχεία. Ποτέ μην κρατάς όλα τα
  ευρήματα στο μυαλό σου μέχρι το τέλος.
- Σε κάθε ενδιάμεση εγγραφή βάλε `:status :partial` και
  `:files-read <όσα ΠΡΑΓΜΑΤΙΚΑ διάβασες>` και
  `:remaining ("ονομαστικά τα αρχεία που ΔΕΝ έχεις ακόμη διαβάσει")`.
- Μόνο στο τέλος γράψε `:status :complete`.
- Αν διακοπείς, το αρχείο σου πρέπει να επιτρέπει σε αντικαταστάτη να συνεχίσει
  ΑΚΡΙΒΩΣ από εκεί που έμεινες — αυτό είναι μέρος του παραδοτέου, όχι ευγένεια.
