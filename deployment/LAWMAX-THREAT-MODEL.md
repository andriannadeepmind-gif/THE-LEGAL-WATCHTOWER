...
| Θ19 | **Correlated / common-control independence failure** (αντίπαλος ελέγχει πολλαπλούς actors που **φαίνονται** ανεξάρτητοι — distinct `kid` — αλλά μοιράζονται κοινό control domain: ίδιος beneficial owner / cloud / key custodian / infrastructure / privileged admin· ένα mesh «independence quorum» καταρρέει σε **ένα** failure domain· self-signed ή unbound «independence» declarations) | D3 evidence-backed independence (v1.5 candidate): χωριστό `IndependenceAssuranceProfile {IA-0/1/2}` (IA-0 δεν μετρά strict)· `ActorIndependenceEvidence/1` **κρυπτογραφικά** δεμένο (actor_kid+public_key+control_domain_id+evidence_subject_digest)· typed `TrustedIssuerRegistry/1`· `revocation_ref` fail-closed· ντετερμινιστικό `control-domain-partition` (union-find· UNKNOWN edge ⇒ union υπό FAIL_CLOSED)· quorum μετρά **distinct control-domain components**, ΟΧΙ distinct `kid`· insufficient ⇒ `INDEPENDENCE_UNKNOWN`. V5KW-D3-7..12 **UNEXECUTED** | ⚠ ΚΕΝΟ → `CHANGE-PROPOSAL-v1.5.md §3/§11.4` + MLTP §15 + `V1.5-SCHEMAS.sexp` (POST-C2, **design-only, CANDIDATE**) |
