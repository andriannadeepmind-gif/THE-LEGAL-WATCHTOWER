...
| Θ18 | **Untrusted input → code execution** (external bytes γίνονται Lisp forms μέσω reader/macro/eval· read-time execution· ontology poisoning) | `SECURE-SEMANTIC-INGRESS-CONTRACT`: external bytes ≠ Lisp forms· sandboxed capability-less parsing· taint states· `*read-eval*` off (όχι μόνη άμυνα)· καμία `read`/`read-from-string` σε untrusted· closed AST· SIK-1..9 predeclared | ⚠ ΚΕΝΟ → `LAWMAX-SECURE-SEMANTIC-INGRESS-CONTRACT.md` (POST-C2, **design-only**) |
