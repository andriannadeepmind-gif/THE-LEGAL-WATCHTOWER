...
| CAP-157 | POST-C2 closure: Public Source-Type Authority Registry — απαριθμήσιμο/επεκτάσιμο/versioned μητρώο ST-01..28 + UNKNOWN_SOURCE_TYPE (SourceTypeSchema/1 vs SourceTypeEntry· ουσιαστικά PENDING_LEGAL_VALIDATION) + source-specific profiles/compilers | UNKNOWN_WITH_OWNER_AND_DEADLINE | `LAWMAX-PUBLIC-SOURCE-TYPE-AUTHORITY-REGISTRY.md` + v1.4 §4.20 | missing | R-132 | owner δημιουργός· census βήμα 1 (U-7 εύρος) |
| CAP-158 | POST-C2 closure: Secure Semantic Ingress — external bytes ≠ Lisp forms· sandboxed parser → `ingress-envelope/1` → non-evaluating JSON/CBOR schema decoder → typed DTO· taint states· SIK-1..9 UNEXECUTED | UNKNOWN_WITH_OWNER_AND_DEADLINE | `LAWMAX-SECURE-SEMANTIC-INGRESS-CONTRACT.md` + **NEW non-evaluating decoder** (ΟΧΙ safe-read· safe-read = internal-only) + v1.4 §4.21 | missing | R-133 | owner δημιουργός· βήμα 2/7 |
