#!/usr/bin/env python3
"""Run golden fixtures (PASS/FAIL) + generated property families against BOTH verification paths — the SBCL
kernel and the independent clingo checker. Each mutated model is rehashed (except the L7-drift fixture) so ONLY
the intended law fails. Every fixture is run through both paths: the checker now derives every model law, so a
one-sided rejection is itself a failure (an undetected disagreement is the defect this gate exists to catch).
Exit 0 iff every golden fixture yields its expected result for its exact reason, every generated property is
rejected, and the two independent paths agree on every fixture."""
...
def sha(fp):
    # one hashing definition everywhere: SHA-256 over the exact raw bytes
...
for fp in sorted(glob.glob(os.path.join(HERE,'FIXTURES','PASS','*.sexp'))+glob.glob(os.path.join(HERE,'FIXTURES','FAIL','*.sexp'))):
    law,exp,reason,mut=parse_fixture(fp)
    check(os.path.basename(fp),law,exp,reason,mut,True); ng+=1
...
for s in subs[:6]:
    ...
    check('gen/L6-nomap/%s'%s,'L6','FAIL','has no requirement->seat->test->WP mapping',['remove-line','requirements-tests-workpackets.sexp',pre],True); np+=1
...
for gid in ('WP-777','WP-888','WP-999'):
    check('gen/L3-dangling/%s'%gid,'L3','FAIL','resolves to no declared wp',['add','requirements-tests-workpackets.sexp','(fact req-map S02__%s :subsystem S02 :requirement R-16 :test Q16 :wp %s)'%(gid,gid)],True); np+=1
...
for st in stores[:4]:
    check('gen/L2-dup/%s'%st,'L2','FAIL','duplicate seat STORE %s'%st,['add','stores-and-authorities.sexp','(fact store %s :owner d :writer d)'%st],True); np+=1
# family: cycle (L4) per back-edge
for a,b in (('PUBLISH','ACQUIRE'),('PROOF','IR'),('COMPILE','ACQUIRE')):
    check('gen/L4-cycle/%s-%s'%(a,b),'L4','FAIL','cycle in the stage-edge graph over stage',['add','dependencies-and-boundaries.sexp','(fact stage-edge %s__%s :from %s :to %s)'%(a,b,a,b)],True); np+=1
