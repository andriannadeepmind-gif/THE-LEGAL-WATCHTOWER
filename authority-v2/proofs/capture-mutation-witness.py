...
MUTANTS = [
    ...
    ("κατάργηση fingerprint ανά αρχείο [ΕΦΕΔΡΙΚΟ — καλύπτεται από τη σταθεροποίηση συνόλου]",
    ...
    ("ΜΕΤΑΒΛΗΤΟ capability (καμία σφράγιση)",
     ...
     '    def __setattr__(self, k, v):\n'
     '        object.__setattr__(self, k, v)  # ΜΕΤΑΛΛΑΞΗ: μεταβλητό capability'),

    ("ΚΑΤΑΣΚΕΥΑΣΙΜΟ capability (χωρίς ιδιωτικό _MINT)",
     '        if token is not _MINT:',
     '        if False:  # ΜΕΤΑΛΛΑΞΗ: οποιοσδήποτε κατασκευάζει ικανότητα'),

    ("ΧΩΡΙΣ ΚΑΡΦΩΜΕΝΟ DIGEST στο canonical profile",
     '    if digest != PINNED_CANONICAL_PROFILE_SHA256:',
     '    if False:  # ΜΕΤΑΛΛΑΞΗ: οποιοδήποτε profile με το σωστό id'),

    ("ΧΩΡΙΣ ΕΠΑΝΕΛΕΓΧΟ ΤΑΥΤΟΤΗΤΑΣ ΑΓΚΥΡΑΣ (stale fd)",
     '    inbox.reverify()          # ΕΠΑΝΕΛΕΓΧΟΣ ΤΑΥΤΟΤΗΤΑΣ πριν από κάθε χρήση\n'
     '    vault.reverify()',
     '    pass  # ΜΕΤΑΛΛΑΞΗ: κανένας επανέλεγχος ταυτότητας'),

    ("ΧΩΡΙΣ ΣΤΑΘΕΡΟΠΟΙΗΣΗ ΣΥΝΟΛΟΥ [ΕΦΕΔΡΙΚΟ — καλύπτεται από το fingerprint ανά αρχείο]",
     '                    _verify_set_stable(src, st, lim, deadline)',
     '                    pass  # ΜΕΤΑΛΛΑΞΗ: καμία σταθεροποίηση συνόλου'),

    ("ΚΑΤΑΡΓΗΣΗ **ΚΑΙ ΤΩΝ ΔΥΟ** ελέγχων μεταβολής (ανά αρχείο ΚΑΙ συνόλου)",
     '            if _fingerprint(os.fstat(fd)) != before:\n'
     '                raise CaptureRefused("mutated-during-capture", child)',
     '            pass  # ΜΕΤΑΛΛΑΞΗ: καμία ανίχνευση μεταβολής'),
]
...
NON_OBSERVABLE = {
    "κατάργηση fingerprint ανά αρχείο [ΕΦΕΔΡΙΚΟ — καλύπτεται από τη σταθεροποίηση συνόλου]":
        "ΑΜΟΙΒΑΙΑ ΕΦΕΔΡΙΚΟΣ με τη σταθεροποίηση συνόλου: ο καθένας ΜΟΝΟΣ ΤΟΥ "
        "ανιχνεύει τον αντίπαλο, άρα κανένας δεν είναι ΑΤΟΜΙΚΑ παρατηρήσιμος. "
        "Η ΚΟΙΝΗ τους κατάργηση ΣΚΟΤΩΝΕΤΑΙ (βλ. μετάλλαξη «ΚΑΙ ΤΩΝ ΔΥΟ»).",
    "ΧΩΡΙΣ ΣΤΑΘΕΡΟΠΟΙΗΣΗ ΣΥΝΟΛΟΥ [ΕΦΕΔΡΙΚΟ — καλύπτεται από το fingerprint ανά αρχείο]":
        "ΑΜΟΙΒΑΙΑ ΕΦΕΔΡΙΚΟΣ με το fingerprint ανά αρχείο — βλ. παραπάνω. Η ΚΟΙΝΗ "
        "κατάργηση ΣΚΟΤΩΝΕΤΑΙ.",
    "κατάργηση του fixed point ΜΕΣΑ στην capture() [ΔΗΛΩΜΕΝΑ ΜΗ ΠΑΡΑΤΗΡΗΣΙΜΗ]":
...
COMBOS = {
    ...
    # Οι δύο έλεγχοι μεταβολής είναι ΕΦΕΔΡΙΚΟΙ: μόνο η ΚΟΙΝΗ τους κατάργηση
    # είναι παρατηρήσιμη. Το προαπαιτούμενο είναι η αφαίρεση της σταθεροποίησης
    # συνόλου· η ίδια η μετάλλαξη αφαιρεί το fingerprint ανά αρχείο.
    "ΚΑΤΑΡΓΗΣΗ **ΚΑΙ ΤΩΝ ΔΥΟ** ελέγχων μεταβολής (ανά αρχείο ΚΑΙ συνόλου)": 19,
}
