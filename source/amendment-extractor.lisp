  ...
  (let* ((handled (make-hash-table :test 'equal))
    ...
    (labels ((codeat (pos)
             ...
             (self-ref-p (code eid)
               ...
               ;; ΜΟΝΟ όταν έγινε προσπάθεια δρομολόγησης (υπάρχει resolver): χωρίς
               ;; resolver, «code=nil» ΔΕΝ σημαίνει «χωρίς ξένη δρομολόγηση» — απλώς
               ;; κανείς δεν ρώτησε· τότε δεν καταπνίγουμε (regression fek-ingestion:
               ;; «άρθρο 2 ΤΟΥ ΠΟΙΝΙΚΟΥ ΚΩΔΙΚΑ» δεν είναι αυτο-αναφορά επειδή τυχαία
               ;; ο ίδιος ο νόμος έχει «Άρθρο 2»).
               (and code-resolver (null code)
